...
public class Teleport implements net.ess3.api.ITeleport {
    ...
    public void cooldown(boolean check) throws Exception {
        ...
        if (teleportOwner.getLastTeleportTimestamp() > 0) {
            ...
            // This value contains the most recent time a teleportPlayer could
            // have been used that would allow another use.
            ...
            if (lastTime > time.getTimeInMillis()) {
                // This is to make sure time didn't get messed up on last
                // teleportPlayer use.
                // If this happens, let's give the user the benifit of the
                // doubt.
    ...
    // The now function is used when you want to skip tp delay when teleporting
    // someone to a location or player.
    @Override
    ...
    public void now(Player entity, boolean cooldown, TeleportCause cause) throws Exception {
        ...
        now(teleportOwner, target, cause);
        teleportOwner.sendMessage(tl("teleporting", target.getLocation().getWorld().getName(),
    ...
    protected void now(IUser teleportee, ITarget target, TeleportCause cause) throws Exception {
        ...
        if (LocationUtil.isBlockUnsafeForUser(teleportee, loc.getWorld(), loc.getBlockX(), loc.getBlockY(),
                loc.getBlockZ())) {
            if (ess.getSettings().isTeleportSafetyEnabled()) {
                ...
                throw new Exception(tl("unsafeTeleportDestination", loc.getWorld().getName(), loc.getBlockX(),
    ...
    // teleportPlayer someone to a location or player.
    ...
    public void teleportPlayer(IUser teleportee, Player entity, Trade chargeFor, TeleportCause cause) throws Exception {
        ...
        teleportee.sendMessage(tl("teleporting", target.getLocation().getWorld().getName(),
                target.getLocation().getBlockX(), target.getLocation().getBlockY(), target.getLocation().getBlockZ()));
        teleportOwner.sendMessage(tl("teleporting", target.getLocation().getWorld().getName(),
                target.getLocation().getBlockX(), target.getLocation().getBlockY(), target.getLocation().getBlockZ()));
    }
    ...
    private void teleport(IUser teleportee, ITarget target, Trade chargeFor, TeleportCause cause) throws Exception {
        ...
        if (chargeFor != null) {
            ...
            // This code is to make sure that commandcosts are checked in the
            // initial world, and not in the resulting world.
            if (!chargeFor.getCommandCost(teleportOwner).equals(BigDecimal.ZERO)) {
                // By converting a command cost to a regular cost, the command
                // cost permission isn't checked when executing the charge after
                // teleport.
    ...
    // The respawn function is a wrapper used to handle tp fallback, on /jail
    // and /home
    @Override
    ...
    public void warp(IUser teleportee, String warp, Trade chargeFor, TeleportCause cause) throws Exception {
        ...
        teleportee.sendMessage(
                tl("warpingTo", warp, loc.getWorld().getName(), loc.getBlockX(), loc.getBlockY(), loc.getBlockZ()));
        if (!teleportee.equals(teleportOwner)) {
            teleportOwner.sendMessage(
                    tl("warpingTo", warp, loc.getWorld().getName(), loc.getBlockX(), loc.getBlockY(), loc.getBlockZ()));
        }
        teleport(teleportee, new LocationTarget(loc), chargeFor, cause);
    }

    // The back function is a wrapper used to teleportPlayer a player /back to
    ...
    public void back(Trade chargeFor) throws Exception {
        ...
        teleportOwner.sendMessage(
                tl("backUsageMsg", loc.getWorld().getName(), loc.getBlockX(), loc.getBlockY(), loc.getBlockZ()));
    ...
    private void initTimer(long delay, IUser teleportUser, ITarget target, Trade chargeFor, TeleportCause cause,
            boolean respawn) {
        timedTeleport = new TimedTeleport(teleportOwner, ess, this, delay, teleportUser, target, chargeFor, cause,
                respawn);
    }
}
