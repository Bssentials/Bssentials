...
public class EssentialsConf extends YamlConfiguration {
    ...
    public synchronized void load() {
        ...
        try {
            try (FileInputStream inputStream = new FileInputStream(configFile)) {
                ...
                if (startSize > Integer.MAX_VALUE) { throw new InvalidConfigurationException("File too big"); }
                ...
                if (result.isError()) {
                    buffer.rewind();
                    data.clear();
                    LOGGER.log(Level.INFO, "File " + configFile.getAbsolutePath() + " is not utf-8 encoded, trying "
    ...
    private void createFromTemplate() {
        ...
        try {
            ...
            try {
                ...
                LOGGER.log(Level.SEVERE, ex + ": Failed to close config: " + configFile.toString());
            }
        }
    }
        ...
        public void run() {
            ...
            synchronized (configFile) {
                if (pendingDiskWrites.get() > 1) {
                    // Writes can be skipped, because they are stored in a queue
                    // (in the executor).
                    // Only the last is actually written.
                    pendingDiskWrites.decrementAndGet();
                    // LOGGER.log(Level.INFO, configFile + " skipped writing in
                    // " + (System.nanoTime() - startTime) + " nsec.");
                    return;
                }
                try {
                    ...
                    if (!configFile.exists()) {
                        try {
                            // TODO LOGGER.log(Level.INFO,
                            // tl("creatingEmptyConfig",
                            // configFile.toString()));
                            LOGGER.log(Level.INFO, "Creating empty config: " + configFile.toString());
                            if (!configFile.createNewFile()) {
                                // TODO LOGGER.log(Level.SEVERE,
                                // tl("failedToCreateConfig",
                                // configFile.toString()));
                                LOGGER.log(Level.INFO, "Failed to create config: " + configFile.toString());
                                return;
                            }
                        } catch (IOException ex) {
                            // TODO LOGGER.log(Level.SEVERE,
                    ...
                    // LOGGER.log(Level.INFO, configFile + " written to disk in
                    // " + (System.nanoTime() - startTime) + " nsec.");
                    pendingDiskWrites.decrementAndGet();
                }
            }
    ...
    public ItemStack getItemStack(final String path) {
        final ItemStack stack = new ItemStack(Material.valueOf(getString(path + ".type", "AIR")),
                getInt(path + ".amount", 1), (short) getInt(path + ".damage", 0));
