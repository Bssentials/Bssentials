...
public class Settings implements net.ess3.api.ISettings {
		...
		if (config.isConfigurationSection("command-costs")) {
			...
			for (String command : section.getKeys(false)) {
				...
				if (section.isDouble(command)) {
					...
					try {
						double cost = Double
								.parseDouble(costString.trim().replace(getCurrencySymbol(), "").replaceAll("\\W", ""));
	...
	// This method should always only return one character due to the implementation
	// of the calling methods
	// If you need to use a string currency, for example "coins", use the
	...
	private Map<Pattern, Long> _getCommandCooldowns() {
		...
		for (String cmdEntry : section.getKeys(false)) {
			...
			if (cmdEntry.startsWith("^")) {
				...
				String cmd = cmdEntry.replaceAll("\\*", ".*"); // Wildcards are accepted as asterisk * as known
				// universally.
