...
public class EssentialsProtectEntityListener implements Listener {
    ...
    public void onEntityDamage(final EntityDamageEvent event) {
        ...
        if (event instanceof EntityDamageByEntityEvent) {
            ...
            // PVP Settings
            if (target instanceof Player && eAttack instanceof Player && prot.getSettingBool(ProtectConfig.disable_pvp)
                    && !user.getName().equalsIgnoreCase(attacker.getName())
                    && (!user.hasPermission("essentials.protect.pvp")
                            || !attacker.hasPermission("essentials.protect.pvp"))) {
                event.setCancelled(true);
                return;
            }
            ...
            if (edEvent.getDamager() instanceof Projectile && target instanceof Player
                    && ((prot.getSettingBool(ProtectConfig.disable_projectiles)
                            && !shouldBeDamaged(user, "projectiles"))
                            || (((Projectile) edEvent.getDamager()).getShooter() instanceof Player
                                    && prot.getSettingBool(ProtectConfig.disable_pvp)
                                    && (!user.hasPermission("essentials.protect.pvp")
                                            || ((Player) ((Projectile) edEvent.getDamager()).getShooter())
                                            .hasPermission("essentials.protect.pvp"))))) {
                event.setCancelled(true);
                return;
            }
        }
    ...
    private boolean shouldBeDamaged(final Player user, final String type) {
        return (user.hasPermission("essentials.protect.damage.".concat(type))
                && !user.hasPermission("essentials.protect.damage.disable"));
    }
    ...
    public void onEntityExplode(final EntityExplodeEvent event) {
        ...
        final int maxHeight = 60;// TODO
        // ess.getSettings().getProtectCreeperMaxHeight();
    ...
    public void onEntityTarget(final EntityTargetEvent event) {
        ...
        if ((event.getReason() == TargetReason.CLOSEST_PLAYER
                ...
                && !user.hasPermission("essentials.protect.entitytarget.bypass")) {
            event.setCancelled(true);
        }
    }
