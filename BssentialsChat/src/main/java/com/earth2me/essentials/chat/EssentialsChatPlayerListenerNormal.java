...
public class EssentialsChatPlayerListenerNormal extends EssentialsChatPlayer {
    public EssentialsChatPlayerListenerNormal(final Server server, final Object ess,
    ...
    public void onPlayerChat(final AsyncPlayerChatEvent event) {
         ...
         * it here
         */
        long radius = 0;// ess.getSettings().getChatRadius();
        if (radius < 1) {
            return;
        }
        radius *= radius;
        ...
        final Player user = chatStore.getUser();
        ...
        if (event.getMessage().length() > 1 && chatStore.getType().length() > 0) {
            ...
            if (user.hasPermission(permission.toString())) {
                ...
                // event.setFormat(tl(chatStore.getType() + "Format",
                // event.getFormat()));
                return;
            }

            // TODO: user.sendMessage(tl("notAllowedTo" +
            // chatStore.getType().substring(0, 1).toUpperCase(Locale.ENGLISH) +
            // chatStore.getType().substring(1)));
        ...
        try {
            ...
            // ess.getLogger().log(Level.INFO, "Plugin triggered custom chat
            // event, local chat handling aborted.", ex);
            // }
            return;
        }
        ...
        while (it.hasNext()) {
            ...
            final Player onlineUser = onlinePlayer;
            if (!onlineUser.equals(user)) {
                ...
                if (abort) {
                    if (onlineUser.hasPermission("essentials.chat.spy")) {
        ...
        if (outList.size() < 2) {
            // TODO user.sendMessage(tl("localNoOne"));
        }

        // LocalChatSpyEvent spyEvent = new
        // LocalChatSpyEvent(event.isAsynchronous(), event.getPlayer(), format,
        // event.getMessage(), spyList);
        // server.getPluginManager().callEvent(spyEvent);

        // if (!spyEvent.isCancelled()) {
        // for (Player onlinePlayer : spyEvent.getRecipients()) {
        // onlinePlayer.sendMessage(String.format(spyEvent.getFormat(),
        // user.getDisplayName(), spyEvent.getMessage()));
        // }
        // }
    }
}
